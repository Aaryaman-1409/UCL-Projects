...
Contributors: Siam Islam, Anelia Gaydardzhieva
Comments: 
...
class MotionInputAPI:
    ...
    def start(cls) -> None:
        ...
        with cls._lock:
            ...
            # Start zeromq client thread if it is enabled in the config file
            if cls._config_editor.get_data("general/zeromq_client_enabled"):
                zeromq_client.start_thread()
                print("[ZeroMQ Client Started]")
                log.info("[[ZeroMQ Client Started]]")
                cls._view_hidden=True
                print("[View Hidden]")
                log.info("[[View Hidden]]")
        ...
        with cls._lock:
            ...
            if cls._active:
                ...
                # Pressed ? for Help - Opens help.txt depending on current_mode (also triggered by saying 'help')
                    ...
                    help_path = os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(__file__)), "data", "help"))
                    mode = cls._mode_editor.get_data("current_mode")
                    # Multitouch/Classic(Basic)
                    if "hand" in mode:
                        target_path = os.path.join(help_path, 'hand', 'help.txt')
                    # FaceNav(Eyes and Nose)
                    elif "eye" in mode or "nose" in mode:
                        target_path = os.path.join(help_path, 'head', 'help.txt')
                    #PseudoVR(Body and Hand)
                    elif "pseudovr" in mode or "fps" in mode:
                        target_path = os.path.join(help_path, 'pseudovr', 'help.txt')
                    # Other modes
                    else:
                        target_path = os.path.join(help_path, 'general', 'help.txt')
                ...
                # Settings MFC GUIs
                    ...
                    mode = cls._mode_editor.get_data("current_mode")
                    version = cls._config_editor.get_data("general/version")
                    ...
                    if target_path != "":
                        subprocess.Popen("start cmd /C " + target_path, cwd=os.getcwd(), shell=True)
...
if __name__ == "__main__":
    ...
    # SystemExit
        ...
        raise
    # All other exceptions, including keyboard exceptions 
