...
class ExtremityCirclesElement(DisplayElement):
    ...
    def update_display(self, image: np.ndarray) -> None:
        ...
        if len(self._extremity_circles_dict) > 0:
            for ((x, y), activated, repeats) in self._extremity_circles_dict.values():
                if activated:
                    rgb = self._colour_dict["green"]
                else:
                    rgb = self._colour_dict["red"]
                image = cv2.circle(image, (x, y), 30, rgb, -1)
                text_size, _ = cv2.getTextSize(str(repeats), self._font, 0.8, 2)
                image = cv2.putText(image, str(repeats), (x - text_size[0] // 2, y + text_size[1] // 2), self._font,
...
class AreaOfInterestElement(DisplayElement):
    ...
    def update(self, width_spacing: Optional[float] = None, height_spacing: Optional[float] = None) -> None:
        ...
        if currentMode.find("fps") != -1:  # walking-fps mode
            config = Config()
            try:
                x = int(config.get_data("events/fps_mode_keys/aoi_coordinates/x"))
                y = int(config.get_data("events/fps_mode_keys/aoi_coordinates/y"))
            except ValueError:
                if currentMode.find("left") != -1:
                    x, y = -150, -100
                else:
                    x, y = 150, -100
