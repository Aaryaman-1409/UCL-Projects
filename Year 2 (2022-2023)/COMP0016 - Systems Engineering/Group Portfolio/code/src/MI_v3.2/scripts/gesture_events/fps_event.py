'''
Author: Jason Ho and Chris Zhang
'''
import datetime
...
config = Config()
mode = ModeEditor()
class FpsModeEvent(ExtremityWalkingEvent):
    """Class representing the walking + extremity triggers fps mode. Special mode to allow for walking to perform an action along with holding an
    ...
    def _set_extremity_keys(self) -> None:
        self._walking_keys = config.get_data("events/fps_mode_keys/walking_extremities")
        self._button_keys = config.get_data("events/fps_mode_keys/button_extremities")
...
class FpsMode1Event(FpsModeEvent):
    """Class for Mode 1 of the Fps mode. The walking D-pad here is used to change the keybind for walking, so
    ...
    """
    _gesture_types = {"walking_left_state","walking_right_state", "extremity_left_walking", "extremity_right_walking", "extremity_up_walking", "extremity_down_walking", "extremity_start_key", "extremity_quit_key"}
    ...
    def set_up(self) -> None:
        ...
        if current_mode.find("left") != -1:
            self._event_triggers["set_displays"](["left_walking_rh", "right_walking_rh", "up_walking_rh", "down_walking_rh", "start_key", "quit_key"])
        else:
            self._event_triggers["set_displays"](["left_walking", "right_walking", "up_walking", "down_walking", "start_key", "quit_key"])
...
class FpsMode2Event(FpsModeEvent):
...
class FpsMode3Event(FpsMode1Event):
    ...
    [gestures types]:
        ...
        "extremity_quit_key": quit key extremity trigger to cause a key down action
    """
    _gesture_types = {"walking_left_state", "walking_right_state", "extremity_left_walking", "extremity_right_walking", "extremity_down_walking", "extremity_start_key", "extremity_quit_key"}
    _event_trigger_types = {"set_displays", "walking_triggered", "extremity_triggered", "held", "walking_released", "extremity_released", "clear"}
    ...
    def _set_extremity_keys(self) -> None:
        self._button_keys = config.get_data("events/fps_mode_keys/button_extremities") | config.get_data("events/fps_mode_keys/walking_extremities")
    ...
    def set_up(self) -> None:
        ...
        current_mode = mode.get_data("current_mode")
        if current_mode.find("left") != -1:
            self._event_triggers["set_displays"](["left_walking_rh", "right_walking_rh", "down_walking_rh", "start_key", "quit_key"])
        else:
            self._event_triggers["set_displays"](["left_walking", "right_walking", "down_walking", "start_key", "quit_key"])