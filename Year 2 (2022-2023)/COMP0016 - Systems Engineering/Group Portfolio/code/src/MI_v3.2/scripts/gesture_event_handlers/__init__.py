...
from scripts.gesture_event_handlers.fps_actions import FpsActions
...
class GestureEventHandlers:
    def __init__(self, mode_change_method: Callable, view):
        ...
        self._handlers = {
            ...
            "FpsActions": self._get_fps_actions_func,
...
# -------------------------------------------------------------------------------------------
# FPS Actions
    def _get_fps_actions_func(self, name: str):
        self._check_fps_actions_initialized()
        funcs = {
            "set_displays": self._fps_actions.set_displays,
            "walking_trigger_action": self._fps_actions.walking_trigger_action,
            "extremity_trigger_action": self._fps_actions.extremity_trigger_action,
            "held_action": self._fps_actions.held_action,
            "extremity_release_action": self._fps_actions.extremity_release_action,
            "walking_release_action": self._fps_actions.walking_release_action,
            "clear_display": self._fps_actions.clear_display}
        if name not in funcs:
            raise RuntimeError("Attempt to get an undefined event handler function:" + name + " from FpsActions")
        return funcs[name]

    def _check_fps_actions_initialized(self):
        self._check_exercise_display_initialized()
        self._check_extremity_circles_initialized()
        if self._fps_actions is None:
            self._fps_actions = FpsActions(self._exercise_display, self._extremity_circles)
