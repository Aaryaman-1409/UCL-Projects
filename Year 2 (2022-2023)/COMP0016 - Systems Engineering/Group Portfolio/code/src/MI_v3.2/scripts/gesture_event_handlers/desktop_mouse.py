...
class AOIMouse(BaseMouse):
    ...
    def move_cursor(self, cam_x: float, cam_y: float):
        ...
        # translate palm center coords to screen coordinates
        if self._currentMode.find("fps") != -1:
            speed = 70

            if self._currentMode.find("left") != -1:
                if cam_x * 640 > 250:
                    set_pos(int(cam_x * speed), 0)
                elif cam_x * 640 < 100:
                    set_pos(-int(cam_x * speed), 0)

            if self._currentMode.find("right") != -1:
                if cam_x * 640 > 540:
                    set_pos(int(cam_x * speed), 0)
                elif cam_x * 640 < 390:
                    set_pos(-int(cam_x * speed), 0)
            ...
            if cam_y * 480 > 160:
                set_pos(0, int(cam_y * speed))
            elif cam_y * 480 < 90:
                set_pos(0, -int(cam_y * speed))

            x, y = self._aoi.convert_xy_fps(cam_x, cam_y)
            old_position = self._cursor_xy
            new_position_x, new_position_y = x, y
            self._mouse_buffer_x.append(new_position_x)
            self._mouse_buffer_x.pop(0)
            self._mouse_buffer_y.append(new_position_y)
            self._mouse_buffer_y.pop(0)
            smoothed_position = (
                int(sum(self._mouse_buffer_x) / len(self._mouse_buffer_x)),
                int(sum(self._mouse_buffer_y) / len(self._mouse_buffer_y)),
            )

            self._cursor_xy = [smoothed_position[0], smoothed_position[1]]

            set_pos(smoothed_position[0] - old_position[0],
                    smoothed_position[1] - old_position[1], )
